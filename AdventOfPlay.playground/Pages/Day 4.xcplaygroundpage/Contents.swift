...
let passwords = input.split(separator: "\n").map { $0.split(separator: " ") }

// Part 1
let validCountPart1 = passwords.filter { $0.count == Set($0).count }.count
print("Found \(validCountPart1) valid passwords using ruleset for part 1");

// Part 2
let validCountPart2 = passwords.filter {
    let charsets = $0.map { Array($0).sorted() }
    return charsets.count == Set(charsets).count
}.count
print("Found \(validCountPart2) valid passwords using ruleset for part 2");
